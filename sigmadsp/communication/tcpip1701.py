...
class SigmaStudioRequestHandler(socketserver.BaseRequestHandler):
    ...
    def handle_write_data(self, packet_header: bytes):
        ...
        total_length = bytes_to_int16(packet_header, 3)
        del total_length
        ...
        # TODO: This field is currently unused.
        chip_address = bytes_to_int8(packet_header, 5) >> 1
        del chip_address

        # The length of the data (uint32).
        total_payload_size = bytes_to_int16(packet_header, 6)

        # The address of the module whose data is being written to the DSP (uint16).
        address = bytes_to_int16(packet_header, 8)
    ...
    def handle_read_request(self, packet_header: bytes):
        ...
        total_length = bytes_to_int16(packet_header, 1)
        del total_length

        # The address of the chip from which the data has to be read
        chip_address = bytes_to_int8(packet_header, 3)
        del chip_address

        # The length of the data (uint32)
        data_length = bytes_to_int16(packet_header, 4)

        # The address of the module whose data is being read from the DSP (uint16)
        address = bytes_to_int16(packet_header, 6)
        ...
        total_transmit_length = 4 + data_length
        ...
        int16_to_bytes(total_transmit_length, transmit_data, 1)
        # int8_to_bytes(chip_address, transmit_data, 3)
